...
接下来证明它的唯一i选哪个。假设存在 $y_0^\prime$ 也满足 $||x_0 - y_0^\prime|| = \delta$，
...
3. $\forall x \in H, \|x\|^2 = \sum_{e\in N} |\langle x, e\rangle|^2$.
4. $\forall x, y \in H, \langle x, y\rangle = \sum_{e\in N}\langle x, e\rangle\langle e, y \rangle$ （Parseval 等式）.

...
色分辨本领：$R = \frac{\lambda}{\delta \lambda} = Nk$，$\delta \lambda$是刚好能分辨的波长差
...
S分量：平行于入射面。P分量：垂直于入射面。

反射光S分量多，折射光P分量多。

当$i = i_0$使得反射光只有S分量，折射光只有P分量。称为布儒斯特角。$i_0 + r_0 = 90\degree$，因而有布儒斯特定律：$\tg i_0 = \frac{n_2}{n_1}$